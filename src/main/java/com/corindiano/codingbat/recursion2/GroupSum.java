...
public class GroupSum {
    ...
    public boolean groupSum(int start, int[] nums, int target) {
        ...
        boolean r = groupSum(start + 1, nums, target - nums[start]);
        if (r) return true;
        return groupSum(start + 1, nums, target);
    }

    // Better solution
    public boolean betterGroupSum(int start, int[] nums, int target) {
        if (start >= nums.length) return (target == 0);

        if (betterGroupSum(start + 1, nums, target - nums[start])) return true;
        if (betterGroupSum(start + 1, nums, target)) return true;
